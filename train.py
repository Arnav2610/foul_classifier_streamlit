...
def create_dataset_from_label_folder(folder, label, processed_files):
    """
    Processes all .mp4 files in the given folder and extracts features.
    Each video is assigned the provided numeric label.
    """
    data = []
...
def main():
    ...
    # For the flop folder (class 0)
    flop_data = create_dataset_from_label_folder('train/flop', label=0, processed_files=processed_files)
    
    # For the foul folder, process each subfolder separately:
    # Warning: class 1, Yellow: class 2, Red: class 3
    warning_data = create_dataset_from_label_folder(os.path.join('train', 'foul', 'warning'), label=1, processed_files=processed_files)
    yellow_data = create_dataset_from_label_folder(os.path.join('train', 'foul', 'yellow'), label=2, processed_files=processed_files)
    red_data = create_dataset_from_label_folder(os.path.join('train', 'foul', 'red'), label=3, processed_files=processed_files)
    
    new_data = flop_data + warning_data + yellow_data + red_data
    if new_data:
    ...
    joblib.dump(clf, 'soccer_classifier.pkl')
    print("Model training complete. Saved as soccer_classifier.pkl.")
    
    # --- Annotate the first training video from the flop folder ---
    first_video = None
    train_flop_folder = os.path.join('train', 'flop')
